...
The package name is `mac-native-screenshare`, version `0.1.0alpha1-1`, for `x86_64`. It targets the environment in [RELEASE_SCOPE.md](RELEASE_SCOPE.md). Installation is inert: no sharing service, automatic startup, credentials, firewall rules, or desktop configuration are installed or activated.
...
The integration source is pinned to the commit defining the preview scope and private runtime paths. Later packaging/documentation commits do not silently change that source snapshot. The two VNC branches remain at their original tested commits. The recipe applies a small, checksum-verified Meson patch to WayVNC solely to set the installed binaries' private library search path.
...
/usr/bin/python3 mac-native-screenshare/packaging/arch/prepare-sources.py \
  --workspace "$PWD" \
  --output "$PWD/package-build/0.1.0alpha1"
cd package-build/0.1.0alpha1
makepkg
/usr/bin/python3 verify-package.py \
  --package ./mac-native-screenshare-0.1.0alpha1-1-x86_64.pkg.tar.zst
```
...
/usr/lib/mac-native-screenshare/
  ...
  run.sh, publish.py, probe.py
  mac-shortcuts.lua, mac-shortcuts.sh, virtual-display.py
/usr/share/doc/mac-native-screenshare/
  README.md, RELEASE_SCOPE.md, ACKNOWLEDGEMENTS.md, sources.json
...
The package checks verify private paths and file modes, required licenses and source metadata, absence of active install scripts, `$ORIGIN` library lookup, use of the bundled NeatVNC, executable smoke checks, and non-overlap with installed stock VNC package files. They do not start a desktop server.

Run `packaging/arch/test-package-install.py <package-file>` from the repository with Python to exercise file installation/removal in an isolated package database/root without installing this preview on the development desktop. This check also needs `fakeroot` and the declared runtime dependencies available on the host. Such a test uses the host libraries for executable checks and does not establish a clean-machine deployment. The user-facing setup and service lifecycle still require steps 4 and 5 and the real second-host/Mac acceptance in step 6.
