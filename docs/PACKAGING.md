...
Download `mac-native-screenshare-0.1.0alpha2-7.src.tar.gz` and `SHA256SUMS` from the [release page](https://github.com/LoFenk/mac-native-screenshare/releases). The separate dependency repositories are not needed after downloading this bundle.

```bash
sha256sum -c SHA256SUMS --ignore-missing
mkdir rebuild
bsdtar -xf mac-native-screenshare-0.1.0alpha2-7.src.tar.gz -C rebuild
cd rebuild/mac-native-screenshare
makepkg --verifysource
makepkg
python verify-package.py --package ./mac-native-screenshare-0.1.0alpha2-7-x86_64.pkg.tar.zst
```
...
/usr/bin/python3 verify-package.py \
  --package ./mac-native-screenshare-0.1.0alpha2-7-x86_64.pkg.tar.zst
```
