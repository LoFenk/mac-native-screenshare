...
Version 0.1.0alpha2-5 is an experimental add-on for x86-64 Omarchy 4.0 / Hyprland 0.56. Setup and service lifecycle are implemented. A second-host Mac connection at scale 2 has been reported; full acceptance remains pending.
...
Revision 4 added a terminal display menu and a guide that can be shown/hidden, plus confirmation before applying a scoped UFW rule during setup. Run `mac-native-screenshare display` to choose normal or temporary desktop sizing, or `display --guide` to read the instructions. Physical scale 1 or 2 remains supported; optional virtual outputs use scale 1.
...
```bash
pacman -Qip ./mac-native-screenshare-0.1.0alpha2-5-x86_64.pkg.tar.zst
