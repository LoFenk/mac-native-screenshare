...
The opt-in `python test-keyboard-live.py --live-desktop --runtime /path/to/runtime` regression check reproduces the input-method case using a private authenticated Unix socket and a scratch Foot terminal. It changes no desktop configuration. The development build passed US→French→US accent and Option+Up checks through the same connection, with Fcitx5 active, and correctly deferred an update while Option was held. All three WayVNC suites passed. The full revision 7 rebuild passed the same isolated suites listed below, binary/source archive verification, and temporary-root revision 6→7 upgrade/removal. The installed package passed accents and Option+Up in Foot and Omarchy agent terminals with the personal workarounds removed. Changing the physical system layout US Macintosh→French→US Macintosh automatically synchronized the same VNC connection; accents and Option+Up passed in each layout, and holding Option deferred the update until release. The original layout was restored, the credential preserved, and all 45 installed files passed integrity verification. Sharing is running. This is live protocol input on the host, not a new second-machine/Mac or reboot acceptance pass. The downloadable revision 7 artifacts use these exact tested implementation snapshots; the release tag includes later packaging and documentation commits.
